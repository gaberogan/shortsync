...
import { FastifyError } from '@api/services/Fastify'
...
export default {
  ...
  handler: async (request, reply) => {
    ...
    if (duration > 60) {
      throw new FastifyError('Video must be 60 seconds or less', 400)
    }
    ...
    const accessToken = await getYoutubeAccessToken(request.user!.email as string)
    ...
    if (requestUploadResponse.status !== 200) {
      throw new FastifyError(await requestUploadResponse.text())
    }
    ...
    // TODO reply.send(uploadResponse) ?
